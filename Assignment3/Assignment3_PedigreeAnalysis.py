...
# Replace "None" with your code
paternal_genotype = converted_genotype[:, 0]  # father
maternal_genotype = converted_genotype[:, 1]  # mother
...
# Get depth slices that match the filtered variants
paternal_depth = depth[paternal_mask][:, 2:]
maternal_depth = depth[maternal_mask][:, 2:]


# Set values with depth < 5 to NaN
paternally_painted[paternal_depth < 5] = np.nan
maternally_painted[maternal_depth < 5] = np.nan
...
for chrom in unique_chromosomes:
    ...
    # Replace "None" with your code
    chrom_mask = (chromosome[paternal_mask] == chrom)
    paternal_slice = paternally_painted[chrom_mask, :]
    ...
    # step
    # YOUR CODE HERE
    chrom_mask_mat = (chromosome[maternal_mask] == chrom)
    maternal_slice = maternally_painted[chrom_mask_mat, :]
...
Based on the visualization, LG10 is the most likely candidate for the sex-determining locus, 
and it is paternally inherited. In the paternal inheritance plot, male offspring show consistently high match_quality values near 1.0, 
while female offspring show much lower values, indicating that the presence of a specific paternal haplotype at LG10 is strongly associated with being male. 
In contrast, the maternal inheritance plot shows no significant sex-specific differences at LG10, supporting the conclusion that this locus is not maternally inherited.
